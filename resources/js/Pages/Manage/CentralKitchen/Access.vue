							...
							<table class="table-auto sm:table-fixed min-w-full w-full">
								<thead>
									<tr>
										...
										<th class="w-1/4 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-left">Name</th>
										<th class="w-1/4 px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-left">Role</th>
										<th v-for="central_kitchen in central_kitchens" :key="central_kitchen.id" class="w-32 px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-center"> {{ central_kitchen.name }}</th>
									</tr>
								</thead>
								<tbody class="bg-white">
									<tr v-for="(user, index) in form.users.filter(user => user.is_admin != true)" :key="user.id" :class="[index % 2 == 0 ? 'bg-white' : 'bg-gray-50', 'border-b']">
										<td class="px-3 py-1 whitespace-wrap break-words">
											<div class="text-sm leading-5 text-gray-700 text-center">{{ (parseInt(index) + 1) }}</div>
										</td>

										<td class="px-3 py-1 whitespace-wrap break-words">
											<Link :href="route('user.edit', user.id)" class="text-sm leading-5 text-blue-600 hover:underline">{{ user.name }}</Link>
										</td>

										<td class="px-3 py-1 whitespace-wrap break-words">
											<div v-if="user.is_admin" class="text-sm leading-5 text-gray-700">Super Admin</div>
											<div v-else class="text-sm leading-5 text-gray-700">{{ user.role_name }}</div>
										</td>

										<td v-for="(central_kitchen) in central_kitchens" :key="central_kitchen.id" class="p-2 whitespace-nowrap text-center">
											<input type="checkbox" v-model="user.central_kitchen_access[central_kitchen.id]" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
										</td>
									</tr>
								</tbody>
							</table>
...
export default {
    ...
    setup (props) {
        ...
        function submit() {
            router.post(route('central.access_update'), form)
        }
