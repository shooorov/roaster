...
import { ArrowTopRightOnSquareIcon, PencilSquareIcon, PlusIcon, UserPlusIcon, TrashIcon } from '@heroicons/vue/24/outline'
            ...
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white shadow sm:rounded-lg">
                    <div class="flex flex-col sm:flex-row sm:justify-between items-center px-4 py-5 border-b border-gray-200 sm:px-8">
                        <p class="max-w-2xl leading-10 text-gray-700 text-lg font-medium mb-4 sm:mb-0">Employees</p>
                    ...
                    <table class="table-auto sm:table-fixed min-w-full w-full" id="table">
                        <thead>
                            <tr>
                                ...
                                <th class="px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-left">Designation</th>
                                <th class="px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-left">Gross</th>
                                <th class="px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-left">Basic</th>
                                <th class="w-32 px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-right">Action</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white">
                            <tr v-for="(employee, index) in employees" :key="employee.id" :class="[index % 2 == 0 ? 'bg-white' : 'bg-gray-50', 'border-b']">
                                <td class="p-2 text-center">{{ index + 1 }}</td>

                                <td class="px-3 py-1 whitespace-wrap break-words">
                                    <div class="flex items-center">
                                        <div>
                                            <div class="text-sm leading-5 font-semibold text-gray-700">{{ employee.name }}</div>
                                            <div class="text-sm leading-5 text-gray-600">{{ employee.email }}</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-3 py-1 whitespace-wrap break-words">
                                    {{ employee.salary_info?.gross }}
                                </td>

                                <td class="px-3 py-1 whitespace-wrap break-words">
                                    {{ employee.salary_info?.basic }}
                                </td>

                                <td class="px-3 py-1 whitespace-wrap break-words">
                                    <div class="flex justify-end">
                                        <Link
                                            v-show="!employee.in_user"
                                            :href="route('employee.user', employee.id)"
                                            class="text-primary-600 hover:text-primary-800 ml-3"
                                            title="add in user">
                                            <UserPlusIcon class="w-6 h-6" aria-hidden="true" />
                                        </Link>
                                        <Link :href="route('employee.edit', employee.id)" class="text-indigo-600 hover:text-indigo-800 ml-3" title="edit">
                                            <PencilSquareIcon class="w-6 h-6" aria-hidden="true" />
                                        </Link>
                                        <button
                                            @click="destroy(route('employee.destroy', employee.id))"
