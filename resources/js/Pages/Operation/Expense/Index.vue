            ...
            <div class="px-4 sm:px-6 lg:max-w-6xl lg:mx-auto lg:px-8">
                <div class="py-6 md:flex md:items-center md:justify-between lg:border-t lg:border-gray-200">
                    ...
                    <div class="mt-6 h-9 flex space-x-3 md:mt-0 md:ml-4">
                        <Link :href="route('expense.create')" class="inline-flex items-center w-full rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white hover:bg-gray-50 text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-primary-400" >
            ...
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white shadow sm:rounded-lg">
                    <div class="flex flex-col sm:flex-row sm:justify-between items-center px-4 py-5 border-b border-gray-200 sm:px-8">
                        <p class="max-w-2xl leading-10 text-gray-700 text-lg font-medium mb-4 sm:mb-0">Expenses ({{ form.start_date.split("-").reverse().join("/") }} - {{ form.end_date.split("-").reverse().join("/") }})</p>
                    </div>
                    ...
                    <table class="table-auto sm:table-fixed min-w-full w-full" id="table">
                        <thead>
                            <tr>
                                ...
                                <th class="w-16 px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-left">Date</th>
                                ...
                                <th class="w-24 px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-right">Amount</th>
                                <th class="w-20 px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-right">Created&nbsp;At</th>
                                <th class="w-12 px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-center">Status</th>
                                <th class="w-20 px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-right">Action</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white">
                            <tr v-for="(expense, index) in expenses" :key="expense.id" :class="[index % 2 == 0 ? 'bg-white' : 'bg-gray-50', 'border-b']">
                                <td class="p-2 text-center"> {{ (index + 1) }} </td>

                                <td class="p-2 whitespace-wrap">
                                    {{ expense.datetime_format }}
                                </td>

                                <td class="p-2 whitespace-wrap">
                                    {{ accounts.find(i => i.id == expense.account_id)?.name }}
                                </td>

                                <td class="p-2 whitespace-wrap">
                                    {{ categories.find(i => i.id == expense.expense_category_id)?.name }}
                                </td>

                                <td class="p-2 whitespace-nowrap text-right px-6">
                                    {{ expense.amount }}
                                </td>

                                <td class="p-2 whitespace-wrap text-right">
                                    {{ expense.created_at_format }}
                                </td>

                                <td class="px-3 py-1 whitespace-wrap break-words">
									<div class="flex justify-center">
										<Status :name="expense.status" :colors="{'unpaid': 'bg-yellow-200 text-yellow-800 rounded', 'paid': 'bg-green-200 text-green-800 rounded'}"></Status>
									</div>
                                </td>

                                <td class="px-3 py-1 whitespace-wrap break-words">
									<div class="py-2 flex flex-col justify-end">
										<div class="flex justify-center">
											<Link v-if="navigation.routes.includes('expense.show')"
												:href="route('expense.show', expense.id)"
												class="text-primary-600 hover:text-primary-800 ml-3" title="detail">
												<ArrowTopRightOnSquareIcon class="w-6 h-6" aria-hidden="true" />
											</Link>
											<Link :href="route('expense.edit', expense.id)" class="text-indigo-600 hover:text-indigo-800 ml-3" title="edit">
												<PencilSquareIcon class="w-6 h-6" aria-hidden="true" />
											</Link>
											<button @click="destroy(route('expense.destroy', expense.id))" class="text-red-600 hover:text-red-800 ml-3" title="delete">
...
export default {
    ...
    setup(props) {
        ...
        function submit() {
            router.visit(route('expense.index'), {
  				data: form,
			});
        }
