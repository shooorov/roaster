            ...
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white shadow sm:rounded-lg">
                    <div class="flex flex-col sm:flex-row sm:justify-between items-center px-4 py-5 border-b border-gray-200 sm:px-8">
                        <p class="max-w-2xl leading-10 text-gray-700 text-lg font-medium mb-4 sm:mb-0">{{ string_change.product_inventory }} Stock</p>

                        <div class="flex-shrink-0 flex space-x-3">
                            <a target="_blank" :href="route('report.product_stock')" class="inline-flex items-center w-full rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white hover:bg-gray-50 text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-0 focus:ring-offset-gray-100 focus:ring-primary-400">
                    ...
                    <table class="table-auto sm:table-fixed min-w-full w-full" id="table">
                        <thead>
                            <tr>
                                ...
                                <th class="px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-left">{{ string_change.product }}</th>
                                <th class="px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-left">Quantity</th>
                                <th class="px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-left">Total Production Price</th>
                                <th class="px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-left">Total Sale Price</th>
                        ...
                        <tbody class="bg-white">
                            <tr v-for="(product, index) in products" :key="product.id" :class="[index % 2 == 0 ? 'bg-white' : 'bg-gray-50', 'border-b']">
                                ...
                                <td class="px-3 py-1 whitespace-wrap break-words">
                                    {{ product.quantity }} <span class="font-mono">pcs</span>
                                </td>

                                <td class="px-3 py-1 whitespace-wrap break-words">
                                    {{ product.production_cost }} <span class="font-mono">&#2547;</span>
                                </td>

                                <td class="px-3 py-1 whitespace-wrap break-words">
                                    {{ product.rate }} <span class="font-mono">&#2547;</span>
                                </td>
                                ...
                                <td class="px-3 py-1 whitespace-wrap break-words">
                                    <div class="flex justify-end">
                                        ...
                                        <Link :href="route('product_inventory.in', {product_id: product.id})" class="text-indigo-600 hover:text-indigo-800 ml-3" title="edit">
...
export default {
    ...
    setup(props) {
        const breadcrumbs = [
            { name: props.string_change.product_inventory + ' Stock', href: route('product_inventory_stock'), current: false },
