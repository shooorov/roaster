                            ...
                            <tr v-for="(delivery, index) in kitchen_deliveries" :key="delivery.id" :class="[index % 2 == 0 ? 'bg-white' : 'bg-gray-50', 'border-b']">
                                ...
                                <td class="px-3 py-1 whitespace-wrap break-words">
                                    <div class="flex justify-end">
                                        <a :href="route('report.kitchen_delivery', delivery.id)" target="_blank" class="text-gray-600 hover:text-gray-800 ml-3" title="print">
                                            <PrinterIcon class="w-6 h-6" aria-hidden="true" />
                                        </a>
                                        <Link :href="route('kitchen_delivery.show', delivery.id)" class="text-primary-600 hover:text-primary-800 ml-3" title="detail">
                                            <ArrowTopRightOnSquareIcon class="w-6 h-6" aria-hidden="true" />
                                        </Link>
                                        <Link :href="route('kitchen_delivery.edit', delivery.id)" class="text-indigo-600 hover:text-indigo-800 ml-3" title="edit">
                                            <PencilSquareIcon class="w-6 h-6" aria-hidden="true" />
                                        </Link>
                                        <button @click="destroy(route('kitchen_delivery.destroy', delivery.id))" class="text-red-600 hover:text-red-800 ml-3" title="delete">
