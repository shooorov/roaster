                ...
                <div class="bg-white shadow sm:rounded-lg">
                    <div class="flex flex-col sm:flex-row sm:justify-between items-center px-4 py-5 border-b border-gray-200 sm:px-8" >
                        ...
                        <div class="flex-shrink-0 flex space-x-3">
                            <!-- <ModelOptions :detailHref="route('other_sale.show', other_sale.id)" :editHref="null" :deleteHref="route('other_sale.destroy', other_sale.id)" deleteMessage="Do you really want to delete this other_sale?" /> -->
                        </div>
                    </div>
                            ...
                            <div class="max-w-5xl mx-auto space-y-4 sm:space-y-6">
                                <div class="md:px-0 py-2 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-1" >
                                        <input v-model="form.date" type="datetime-local" class="focus:ring-indigo-400 focus:border-indigo-400 hover:bg-gray-100 focus:bg-transparent block w-full px-4 sm:text-sm border-gray-300 rounded" />
                                        <InputError :message="$page.props.errors.date" />
                                    </dd>
                                </div>

                                <div class="md:px-0 py-2 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                    <dt class="text-sm leading-10 font-semibold text-gray-700 tracking-wider text-right pr-8" > Branch <span class="text-red-500">*</span> </dt>
                                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-1" >
                                ...
                                <div class="md:px-0 py-2 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-1" >
                                        <Combobox class="mt-1" v-model="form.other_sale_category_id" :items="other_sale_categories" />
                                        <InputError :message="$page.props.errors.other_sale_category_id" />
                                    </dd>
                                </div>
                                ...
                                <div class="md:px-0 py-2 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                    <dt class="text-sm sm:leading-10 font-semibold text-gray-700 tracking-wider sm:text-right pr-8"> Description </dt>
                                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                        <textarea v-model="form.description" type="text" placeholder="Description Here" rows="4" class="focus:ring-indigo-400 focus:border-indigo-400 hover:bg-gray-100 focus:bg-transparent block w-full px-4 sm:text-sm border-gray-300 rounded"></textarea>
...
export default {
    ...
    setup(props) {
        ...
        const form = reactive({
            date: props.other_sale.date_format,
            amount: props.other_sale.amount,
            check_no: props.other_sale.check_no,
            account_id: props.other_sale.account_id,
            branch_id: props.other_sale.branch_id,
            other_sale_category_id: props.other_sale.other_sale_category_id,
            description: props.other_sale.description,
        });

        function submit() {
            router.patch(route("other_sale.update", props.other_sale.id), form);
        }
