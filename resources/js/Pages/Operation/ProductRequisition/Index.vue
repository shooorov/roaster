<template>
	<Head> <title> {{ string_change.product }} Requisitions </title> </Head>
            ...
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white shadow sm:rounded-lg">
                    <div class="flex flex-col sm:flex-row sm:justify-between items-center px-4 py-5 border-b border-gray-200 sm:px-8">
                        <p class="max-w-2xl leading-10 text-gray-700 text-lg font-medium mb-4 sm:mb-0">{{ string_change.product }} Requisitions ({{ form.start_date }} - {{ form.end_date }})</p>
                    ...
                    <table class="table-auto sm:table-fixed min-w-full w-full" id="table">
                        <thead>
                            <tr>
                                ...
                                <th class="px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-left">Grand Total</th>
                                <th class="w-40 px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-right">Action</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white">
                            <tr v-for="(requisition, index) in requisitions" :key="requisition.id" :class="[index % 2 == 0 ? 'bg-white' : 'bg-gray-50', 'border-b']">
                                <td class="p-2 text-center"> {{ (index + 1) }} </td>

                                <td class="px-3 py-1 whitespace-wrap break-words"> {{ requisition.datetime_format }} </td>

                                <td class="px-3 py-1 whitespace-wrap break-words"> {{ requisition.total ?? 0 }} </td>

                                <td class="px-3 py-1 whitespace-wrap break-words">
                                    <div class="flex justify-end">
                                        <a :href="route('report.product_requisition', requisition.id)" target="_blank" class="text-gray-600 hover:text-gray-800 ml-3" title="print">
                                            <PrinterIcon class="w-6 h-6" aria-hidden="true" />
                                        </a>
                                        <Link :href="route('product_requisition.show', requisition.id)" class="text-primary-600 hover:text-primary-800 ml-3" title="detail">
                                            <ArrowTopRightOnSquareIcon class="w-6 h-6" aria-hidden="true" />
                                        </Link>
                                        <Link :href="route('product_requisition.edit', requisition.id)" class="text-indigo-600 hover:text-indigo-800 ml-3" title="edit">
                                            <PencilSquareIcon class="w-6 h-6" aria-hidden="true" />
                                        </Link>
                                        <button @click="destroy(route('product_requisition.destroy', requisition.id))" class="text-red-600 hover:text-red-800 ml-3" title="delete">
...
export default {
    ...
    setup(props) {
        ...
        function submit() {
            router.visit(route('product_requisition.index'), {
  				data: form,
			});
        }
