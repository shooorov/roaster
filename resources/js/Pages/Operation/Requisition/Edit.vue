            ...
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white shadow sm:rounded-lg">
                    <div class="flex flex-col sm:flex-row sm:justify-between items-center px-4 py-5 border-b border-gray-200 sm:px-8">
                        <p class="max-w-2xl leading-10 text-gray-700 text-lg font-medium mb-4 sm:mb-0">{{ string_change.item }} Requisition Edit</p>

                        <div class="flex-shrink-0 flex space-x-3">
                            <ModelOptions :detailHref="route('requisition.show', requisition.id)" :editHref="null" :deleteHref="route('requisition.destroy', requisition.id)" deleteMessage="Do you really want to delete this requisition?" />
                        </div>
                    </div>
                                ...
                                <table class="table-auto sm:table-fixed min-w-full divide-y divide-gray-200">
                                    ...
                                    <tfoot class="bg-white">
                                        <tr>
                                            ...
                                            <th colspan="1" scope="col" class="py-2 text-center text-lg font-medium font-mono">{{ form.total }}</th>
                                        </tr>
                                    </tfoot>
                                </table>
                            </div>
...
export default {
    ...
    setup(props) {
        ...
        const form = reactive({
            ...
            total: props.requisition.group_items.reduce((carry, val) => carry + Number(val.total), 0),
            group_items: props.requisition.group_items,
        })

        function submit() {
            router.patch(route('requisition.update', props.requisition.id), form)
        }
