<template>
    <Head :title="'Edit ' + string_change.item + ' Category'" />
            ...
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white shadow sm:rounded-lg">
                    <div class="flex flex-col sm:flex-row sm:justify-between items-center px-4 py-5 border-b border-gray-200 sm:px-8">
                        <p class="max-w-2xl leading-10 text-gray-700 text-lg font-medium mb-4 sm:mb-0"> {{ string_change.item }} Category Edit</p>
                        ...
                        <dl class="space-y-4 sm:space-y-6 px-5 py-6">
                            ...
                            <!-- <div class="grid sm:grid-cols-3 sm:gap-2 sm:px-6">
                                <dt class="text-sm sm:leading-10 font-semibold text-gray-700 tracking-wider sm:text-right pr-8">
                                     {{ string_change.item }} Category
                                </dt>
                                <dd class="mt-1 text-sm text-gray-700 sm:mt-0 sm:col-span-1">
                                    <Combobox class="mt-1" v-model="form.item_category_id" :items="item_categories" />
                                </dd>
                            </div> -->
...
export default {
    ...
    setup (props) {
        ...
        const form = reactive({
            item_category_id: props.item_category.item_category_id,
            name: props.item_category.name,
        })

        function submit() {
            router.patch(route('item_category.update', props.item_category.id), form)
        }
