            ...
            <div class="px-4 sm:px-6 lg:max-w-6xl lg:mx-auto lg:px-8">
                <div class="py-6 md:flex md:items-center md:justify-between lg:border-t lg:border-gray-200">
                    ...
                    <div class="mt-6 h-9 flex space-x-3 md:mt-0 md:ml-4">
                        <Link :href="route('expense_category.create')" class="inline-flex items-center w-full rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white hover:bg-gray-50 text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-primary-400">
            ...
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white shadow sm:rounded-lg">
                    <div class="flex flex-col sm:flex-row sm:justify-between items-center px-4 py-5 border-b border-gray-200 sm:px-8">
                        <p class="max-w-2xl leading-10 text-gray-700 text-lg font-medium mb-4 sm:mb-0"> Expense Category Edit</p>
                    ...
                    <form @submit.prevent="submit">
                        <dl class="space-y-4 sm:space-y-6 px-5 py-6">
                            ...
                            <div class="grid sm:grid-cols-3 sm:gap-2 sm:px-6">
                                ...
                                <dd class="flex mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2 gap-3">
                                    <label class="mb-5 inline-flex items-center">
                                        <input v-model="form.type" value="receive" type="radio" class="form-radio w-5 h-5 border-gray-300 text-indigo-500 cursor-pointer focus:ring-indigo-500">
                                        <span class="ml-2 tracking-wide cursor-pointer"> Receive </span>
                                    </label><br>
                                    <label class="mb-5 inline-flex items-center">
                                        <input v-model="form.type" value="payment" type="radio" class="form-radio w-5 h-5 border-gray-300 text-indigo-500 cursor-pointer focus:ring-indigo-500">
                                        <span class="ml-2 tracking-wide cursor-pointer"> Payment </span>
                                    </label><br>
...
export default {
    ...
    setup (props) {
        const breadcrumbs = [
            { name: 'Expense Categories', href: route('expense_category.index'), current: false },
            { name: 'Edit Page', href: '#', current: false },
        ]

        const form = reactive({
            name: props.expense_category.name,
            type: props.expense_category.type,
            expense_category_id: props.expense_category.expense_category_id,
        })

        function submit() {
            router.patch(route('expense_category.update', props.expense_category.id), form)
        }
