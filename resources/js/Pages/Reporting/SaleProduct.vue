			...
			<div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
				<div class="bg-white shadow sm:rounded-lg">
					<div
                        ...
                        <p class="max-w-2xl leading-10 text-gray-700 text-lg font-medium mb-4 sm:mb-0">Sale Product Summary ({{ form.start_date.split("-").reverse().join("/") }} - {{ form.end_date.split("-").reverse().join("/") }})</p>

                        <div class="flex-shrink-0 flex space-x-3">
							<a :href="route('export.sale_products', form)" type="button" v-if="navigation.routes.includes('export.sale_products')" class="inline-flex items-center gap-x-1.5 rounded-md bg-blue-50 px-3 py-2 text-sm font-semibold text-blue-700 shadow-sm hover:bg-blue-100 border border-blue-100">
					...
					<table class="table-auto sm:table-fixed min-w-full w-full" id="table">
						...
						<tbody class="bg-white">
							<tr v-for="(product, index) in sold_products" :key="product.id"
								:class="[index % 2 == 0 ? 'bg-white' : 'bg-gray-50', 'border-b']">
								<td class="p-2 text-center"> {{ (index + 1) }} </td>

								<td class="p-2 whitespace-wrap">
									<div class="text-sm leading-5 text-gray-700">{{ product.name }}</div>
								</td>
...
export default {
	...
	setup(props) {
		const breadcrumbs = [
			...
			{ name: 'Product Summary', href: '#', current: false },
		];
		...
		function submit() {
            router.visit(route('summary.sale_products'), {
  				data: form,
			});
		}
